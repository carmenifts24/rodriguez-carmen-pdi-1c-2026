import py5

img = None

def setup():
    global img
    py5.size(800, 400)
    img = py5.load_image("img/imagen.jpg")
    img.resize(400, 400)

def draw():
    py5.background(35)

    # Imagen original en la mitad izquierda (sin modificar)
    py5.image(img, 0, 0)

    # Calcular el factor de ajuste según la posición X del mouse
    # remap convierte un valor de un rango a otro
    # Acá: de 0 a 800 píxeles de ancho → de 0 a 2.5 de factor multiplicador
    #factor_rojo = py5.remap(py5.mouse_x, 0, py5.width, 0, 2.5)
    #factor_verde = py5.remap(py5.mouse_x, 0, py5.width, 0, 2.5)
    factor_azul = py5.remap(py5.mouse_x, 0, py5.width, 0, 2.5)


    # Acceder a la matriz de píxeles del lienzo completo
    img.load_pixels()
    py5.load_pixels()

    for x in range(img.width):
        for y in range(img.height):

            # La imagen es un arreglo lineal. Para acceder al píxel (x, y):
            # índice = x + y * ancho
            indice_img = x + y * img.width
            pixel = img.pixels[indice_img]

            # Separar los canales
            r = py5.red(pixel)
            g = py5.green(pixel)
            b = py5.blue(pixel)

            # Modificar solo el canal rojo según el mouse
            #r = r * factor_rojo
            #r=0
            #g = g * factor_verde
            b = b * factor_azul

            # Limitar el valor para que no supere 255
            # Un valor mayor haría que py5 lo interprete incorrectamente
            if r > 255:
                r = 255

            # Calcular el índice del mismo píxel en el lienzo (desplazado 400px a la derecha)
            indice_canvas = (x + 400) + y * py5.width
            py5.pixels[indice_canvas] = py5.color(r, g, b) 
            #py5.pixels[indice_canvas] = py5.color(b, g, r)
            #py5.pixels[indice_canvas] = py5.color(g, r, b)

    # Aplicar los cambios al lienzo
    py5.update_pixels()

# Resumen de los cambios de prueba hechos en este archivo:
# Línea 22: se activó factor_azul en lugar de factor_rojo o factor_verde.
# Esto hace que la posición X del mouse controle la intensidad del canal azul.
# Líneas 43-46: se dejaron comentadas las alternativas para modificar rojo o verde,
# y se activó b = b * factor_azul. De esta manera, el canal azul se amplifica
# o reduce según la posición horizontal del mouse.
# Líneas 50-51: el recorte sigue aplicado sobre r. Si se quiere trabajar de forma
# consistente con el canal azul, el clipping debería pasar a hacerse sobre b.
# Líneas 55-57: se mantiene activa la composición normal py5.color(r, g, b) y
# quedan comentadas dos variantes para experimentar cambiando el orden de canales.

py5.run_sketch()
